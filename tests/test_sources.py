...
def docker_source_app():
    ...
    return rbcore.create_app('test', **config)

def test_dockerSource_defaults(docker_source_app):
    with docker_source_app.app_context():
        test_source = rbcore.source.source.init('test-source-docker')
        ...
        with pytest.raises(rbcore.errors.SourceError):
            test_source.create()
...
def test_source_model(app):
    with app.app_context():
        ...
        test_source_model = rbcore.source.model.Sources.find_one(**{'name': 'test-source-model'})
        with pytest.raises(rbcore.errors.SourceError):
            test_source_model.create()
        test_source_model = rbcore.source.model.Sources.update(test_source_model)
        assert test_source_model.status == 'stopped'
        test_source_model = rbcore.source.model.Sources.update('test-source-model', **{'stream_mountpoint': 'ladida'})
        assert test_source_model.stream_mountpoint == 'ladida'
        test_source_model = rbcore.source.model.Sources.delete(test_source_model)
        assert test_source_model.status == 'non-existent'
        with pytest.raises(rbcore.errors.DatabaseNotFound):
            rbcore.source.model.Sources.find_one(**{'name': 'test-source-model'})
