...
def docker_source_app():
    ...
    return rbteam.create_app('test', **config)

def test_dockerSource_defaults(docker_source_app):
    with docker_source_app.app_context():
        test_source = rbteam.source.source.init('test-source-docker')
        ...
        with pytest.raises(rbteam.errors.SourceError):
            test_source.create()
...
def test_source_model(app):
    with app.app_context():
        test_source_models = rbteam.source.model.Sources.find()
        assert test_source_models == []
        test_source_model = rbteam.source.model.Sources.create(**{'name': 'test-source-model', 'channel': 'dumb'})
        ...
        test_source_model = rbteam.source.model.Sources.find_one(**{'name': 'test-source-model'})
        with pytest.raises(rbteam.errors.SourceError):
            test_source_model.create()
        test_source_model = rbteam.source.model.Sources.update(test_source_model)
        assert test_source_model.status == 'stopped'
        test_source_model = rbteam.source.model.Sources.update('test-source-model', **{'stream_mountpoint': 'ladida'})
        assert test_source_model.stream_mountpoint == 'ladida'
        test_source_model = rbteam.source.model.Sources.delete(test_source_model)
        assert test_source_model.status == 'non-existent'
        with pytest.raises(rbteam.errors.DatabaseNotFound):
            rbteam.source.model.Sources.find_one(**{'name': 'test-source-model'})
